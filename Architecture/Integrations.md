...
| FreeRDP pointer and keyboard control | BrainConnect | Keyboard launch effects independently verified; connector installed inert; cursor and frame verification pending | Exact endpoint, NLA/HYBRID, and certificate recheck; target-local credential binding; console-session reconnect; slow-path bounded pointer, text, key, and multi-step keyboard input; all redirections disabled | [Open-lab control](../../BrainConnect/docs/OPEN_LAB.md) |
...
| Hyper-V Windows lab | BrainConnect / BoxBrain | Installed, checkpointed, independently identified, registered, enabled, and restore-tested | Generation 2 Windows 11 VM on Pi-only USB network; guarded UAC-gated restore and no personal account or files | [Windows lab runbook](../sandbox/hyperv/README.md) |
