...
Aurum contributor lanes are read-only against the upstream Aurum source and should return test/build evidence only. They do not have authority to promote code into the trusted Aurum generation.
...
Aurum prefers useful parallel work over duplicate work. Contributor capacity is most valuable when it verifies a different architecture, test shard, implementation candidate, or failure mode. The trusted Aurum promotion path still requires verified convergence before changes reach physical nodes.
