...
The verifier consumes a live emulator-only probe, independently reconciled
register bindings, and independently reconciled transition evidence. It can also
compare Aurum's virtual-only execution for a synthesized DLAB program with the
live QEMU observation. It never opens a host device or performs physical I/O.
"""
...
def _verified_transition(model: dict[str, Any], key: str) -> dict[str, Any]:
    ...
    if not isinstance(transition, dict):
        raise ValueError(f"verified transition has no transition body: {key}")
    return transition
...
def verify_emulator_parity(
    binding_model: dict[str, Any],
    transition_model: dict[str, Any],
    probe: dict[str, Any],
    *,
    virtual_dlab_execution: dict[str, Any] | None = None,
) -> dict[str, Any]:
    """Fail closed unless live QEMU observations match independently learned behavior."""
    ...
    for key in (
        "host_physical_io_performed",
        "host_device_file_io_performed",
        "physical_writes_performed",
        "firmware_changes_performed",
    ):
    ...
    selector_checks = {
        "receiver_or_divisor_lsb": rb.get("offset") == dll.get("offset") == probe.get("selector_offsets", {}).get("receiver_or_divisor_lsb"),
        "interrupt_or_divisor_msb": ier.get("offset") == dlm.get("offset") == probe.get("selector_offsets", {}).get("interrupt_enable_or_divisor_msb"),
        "line_control": lcr.get("offset") == probe.get("selector_offsets", {}).get("line_control"),
        "line_status": lsr.get("offset") == probe.get("selector_offsets", {}).get("line_status"),
        "bank_requirements": rb.get("dlab") == 0 and ier.get("dlab") == 0 and dll.get("dlab") == 1 and dlm.get("dlab") == 1,
    }

    reset = _verified_transition(transition_model, "reset.line_status_defaults")
    reset_after = reset.get("after", {})
    expected_lsr = 0
    if reset_after.get("lsr.data_ready") is True:
        expected_lsr |= dr_mask
    if reset_after.get("lsr.thre") is True:
        expected_lsr |= thre_mask
    if reset_after.get("lsr.temt") is True:
        expected_lsr |= temt_mask

    dlab = _verified_transition(transition_model, "dlab.select_divisor_latches")
    if dlab.get("after", {}).get("lcr.dlab") is not True:
    ...
    expected_ports = {
        "line_status": base_port + lsr["offset"],
        "line_control": base_port + lcr["offset"],
        "divisor_lsb": base_port + dll["offset"],
        "divisor_msb_or_ier": base_port + dlm["offset"],
    }
    port_checks = {
        "line_status": obs_lsr["port"] == expected_ports["line_status"],
        "line_control_reset": obs_lcr_reset["port"] == expected_ports["line_control"],
        "line_control_set": obs_lcr_set["port"] == expected_ports["line_control"],
        "divisor_lsb": obs_dll["port"] == expected_ports["divisor_lsb"],
        "divisor_msb": obs_dlm["port"] == expected_ports["divisor_msb_or_ier"],
        "line_control_clear": obs_lcr_clear["port"] == expected_ports["line_control"],
        "interrupt_enable": obs_ier["port"] == expected_ports["divisor_msb_or_ier"],
    }
    ...
    if virtual_dlab_execution is not None:
        ...
        virtual_safety = virtual_dlab_execution.get("safety", {})
        if virtual_safety.get("hardware_access_performed") is not False or virtual_safety.get("physical_writes_performed") is not False:
            raise ValueError("virtual DLAB execution escaped its safety boundary")
        virtual_value = virtual_dlab_execution.get("final_logical_registers", {}).get("selector.line_control")
        virtual_checks = {
            "aurum_virtual_dlab_matches_live_qemu": virtual_value == obs_lcr_set["value"],
        }
    ...
    result = {
        ...
        "observed": {
            "reset_line_status": obs_lsr["value"],
            "dlab_set_line_control": obs_lcr_set["value"],
            "divisor_lsb": obs_dll["value"],
            "divisor_msb": obs_dlm["value"],
            "ier_after_bank_restore": obs_ier["value"],
        },
        "safety": {
            "physical_hardware_access_performed": False,
            "physical_writes_performed": False,
            "firmware_changes_performed": False,
            "qemu_emulated_io_only": True,
        },
    }
...
def build_models_and_virtual_dlab(repo_root: Path) -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
    ...
    dlab_programs = [
        program for program in program_set["programs"]
        if any(step.get("transition_key") == "dlab.select_divisor_latches" for step in program.get("steps", []))
    ]
    if len(dlab_programs) != 1:
        raise ValueError("DLAB transition must map to exactly one abstract program")
    virtual = execute_virtual_program(
        plan,
        dlab_programs[0]["program_identity"],
        logical_registers={"selector.line_control": 0},
    )
