...
def diagnose_native_synthesis_failure(parameters: Sequence[str], examples: Sequence[NativeExample]) -> NativeFailureDiagnosis:
    ...
    expected=tuple(e.expected for e in examples); target_types={_value_type(v) for v in expected}; target_type=next(iter(target_types)) if len(target_types)==1 else "mixed"
