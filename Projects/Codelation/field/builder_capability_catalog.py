...
CATALOG_REVISION = "aurum-builder-capability-catalog-v7"
...
def default_builder_capabilities() -> tuple[BuilderCapabilityDescriptor, ...]:
    ...
    return (
        ...
        BuilderCapabilityDescriptor("bounded-preference-evidence","preference_evidence","project_preference_evidence",frozenset({"explicit-feedback-precedence","bounded-preference-evidence-projection","neutral-feedback-preservation"}),pure|frozenset({"local-learning-only","no-trait-inference","no-actuation"}),"none","preference-evidence-v0"),
    )


def get_builder_capability(name: str) -> BuilderCapabilityDescriptor | None:
    return next((d for d in default_builder_capabilities() if d.name==name),None)
...
def find_builder_capability_candidates(requirements: Iterable[str], *, catalog: Iterable[BuilderCapabilityDescriptor] | None=None) -> tuple[BuilderCapabilityCandidate,...]:
    ...
    for d in catalog or default_builder_capabilities():
        ...
        missing=tuple(sorted(required-d.provides))
        out.append(BuilderCapabilityCandidate(d.name,d.module,d.callable_name,matched,missing,len(matched)/len(required),d.authority,d.verification_adapter))
