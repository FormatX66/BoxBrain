...
class DriverSynthesisTests(unittest.TestCase):
    ...
    def test_duplicate_same_kind_cannot_outvote_independent_sources(self):
        claims = [EvidenceClaim("reset.ready_value", 1, "reference_driver", f"driver-copy-{i}", 1.0) for i in range(20)]
        claims.extend([
        ...
        self.assertEqual("verified", model["claims"]["reset.ready_value"]["state"])
        self.assertEqual(0, model["claims"]["reset.ready_value"]["value"])

    def test_single_source_claim_remains_uncertain(self):
        model = reconcile_evidence([EvidenceClaim("undocumented.mode", "x", "reference_driver", "one-driver", 1.0)])
    ...
    def test_invalid_or_unbounded_evidence_is_rejected(self):
        with self.assertRaises(ValueError):
            reconcile_evidence([EvidenceClaim("register.x", 1, "unknown-source", "mystery", 1.0)])
        with self.assertRaises(ValueError):
            reconcile_evidence([EvidenceClaim("register.x", 1, "datasheet", "manual", 1.01)])
        with self.assertRaises(ValueError):
            reconcile_evidence([EvidenceClaim("", 1, "datasheet", "manual", 1.0)])
    ...
    def test_model_identity_is_deterministic_across_input_order(self):
        ...
        self.assertEqual(synthesize_candidate_interface(first)["candidate_identity"], synthesize_candidate_interface(second)["candidate_identity"])
    ...
    def test_real_public_uart_bundle_reconstructs_correlated_behavior(self):
        ...
        self.assertEqual("receive-data-available", model["claims"]["line_status.data_ready.meaning"]["value"])
        self.assertEqual("transmit-holding-register-empty", model["claims"]["line_status.thre.meaning"]["value"])
        self.assertEqual("uncertain", model["claims"]["line_control.dlab.meaning"]["state"])
        candidate = synthesize_candidate_interface(model)
        self.assertIn("linux:drivers/tty/serial/8250/8250_port.c", candidate["reference_driver_teachers"])
        self.assertFalse(candidate["promotion_gates"]["physical_write_authorized"])
    ...
    def test_public_uart_register_bindings_are_independently_verified(self):
        ...
        self.assertEqual(13, len(model["claims"]))
        ...
        self.assertEqual({"offset": 4, "dlab": "any"}, model["claims"]["selector.modem_control"]["value"])
        ...
        self.assertEqual(16, model["claims"]["mask.modem_control.loop"]["value"])
        ...
        candidate = synthesize_candidate_interface(model)
        self.assertEqual(13, len(candidate["resolved_claims"]))
    ...
    def test_uncertain_claim_in_trace_cannot_be_upgraded_by_replay(self):
        model = self._public_uart_model()
        trace = json.loads(TRACE.read_text(encoding="utf-8"))
        trace["events"].append({"step": 4, "claim_key": "line_control.dlab.meaning", "observed_value": "divisor-latch-access"})
        verification = verify_behavior_trace(model, trace)
