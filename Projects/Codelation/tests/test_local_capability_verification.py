...
class LocalCapabilityVerificationTests(unittest.TestCase):
    ...
    def test_reversible_delta_projection_satisfies_adaptation_proposal_without_authority(self):
        gap = get_native_semantic_gap("interface_adaptation_proposal")
        self.assertIsNotNone(gap)
        verification = verify_local_capability_for_gap(gap, "reversible-state-delta-projection")
        self.assertTrue(verification.verified)
        self.assertEqual(verification.passed, verification.examples)
        self.assertEqual(verification.invocation_output, "add=terminal;remove=none;evidence=coding-confidence-high")
        self.assertFalse(verification.authority_granted)
        self.assertFalse(verification.routed_to_host)
