...
- The refreshed fallback matrix on draft PR #83, head `ef137007d04a8d511ab8cdee6b3699ba0ad132a6`, now has a **canonical-provenance guard** in addition to the prior virtual boot proof. The provenance workflow requires the protected Germ, genetics, offline-carrier policy, recovery logic, and recovery trust inputs to match the canonical `READY_TO_FLASH` release source while still allowing the carrier/loader method to differ. Provenance workflow run `32829696524` passed. Fallback-matrix run `32829696269` passed the protected Germ suite, built both raw-GPT candidates, cryptographically reverified the embedded offline phenotype from each finished raw image, and passed OVMF removable-media boot smoke for both loader families while requiring Tiny Seed ready, boot-proof, and network-ready markers. It published `Aurum-TinySeed-amd64-fallback-matrix-experimental` as artifact id `9556671977`, digest `sha256:c53abb3c670848f852a42418d3689c5ce7472808c43fd9f79335c0879a22ff38`. Repository-integrity, Germ safety, Future Branch, Codelation, and backend CircleCI lanes also passed on this head. This is **experimental Passed + Published + provenance-locked** evidence only; it does not promote either fallback, grant write authority, or claim physical HP compatibility.
