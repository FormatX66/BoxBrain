...
[Tiny Seed boot-medium lifecycle](../../docs/architecture/TINY_SEED_BOOT_MEDIUM.md).
...
The exact kernel currently selected on the sacrificial Pi 3 trial card has an
evidence-only `raspi3b` boot-to-init receipt at
`Projects/AdaptiveKernel/results/pi3-architecture-accurate-boot-latest.json`.
That receipt proves the board-model boot boundary only. It does not prove the
physical LAN9514/`smsc95xx` path, external recovery, or mutation authority.
