...
5. the saved Wi-Fi profile set is on durable storage and reconnects with the
   same fingerprints after a later boot;
6. the existing bounded GPT and system-integration proofs pass.

If physical input or reboot evidence is still pending, the generation remains
installed as reversible `applied-awaiting-proof`; it is not promoted. A hard
proof failure culls the candidate, heals the previous running seed from its
displaced-state bundle, and requires a new forward successor. Published Git
history is never moved backward.
