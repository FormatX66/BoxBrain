...
def network_repair() -> dict[str, Any]:
    ...
    for name in _wired_interfaces():
        actions.append({"action": f"link-up:{name}", **_bounded_command(["ip", "link", "set", "dev", name, "up"], timeout=5)})
        actions.append({"action": f"reconfigure:{name}", **_bounded_command(["networkctl", "reconfigure", name], timeout=8)})
        actions.append({"action": f"renew:{name}", **_bounded_command(["networkctl", "renew", name], timeout=8)})
...
def main() -> int:
    ...
    while True:
        ...
        if command in {"help", "?"} and len(tokens) <= 1:
            ...
            print(json.dumps(network_repair(), indent=2, sort_keys=True), flush=True)
