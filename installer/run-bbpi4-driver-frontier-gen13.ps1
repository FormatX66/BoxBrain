...
$extensionPath = Join-Path $PSScriptRoot 'aurum-pi4-driver-gen13-extension.ps1'
...
$limitNew = 'elseif ($generation -eq $validated -and $next -eq ($validated + 1) -and $next -le 13) {'
if (-not $patched.Contains($limitOld)) {
    throw 'Could not extend the Aurum Pi4 autonomous next-generation limit to generation 13.'
}
$patched = $patched.Replace($limitOld, $limitNew)

$guardOld = 'if ($trialGeneration -lt 1 -or $trialGeneration -gt 10) {'
$guardNew = 'if ($trialGeneration -lt 1 -or $trialGeneration -gt 13) {'
if (-not $patched.Contains($guardOld)) {
    throw 'Could not extend the Aurum Pi4 supported-generation guard to generation 13.'
}
