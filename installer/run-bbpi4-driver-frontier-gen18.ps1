...
$extensionPath = Join-Path $PSScriptRoot 'aurum-pi4-driver-gen18-extension.ps1'
$frontierPath = Join-Path $repoRoot 'Projects\Codelation\driver_evidence\pi4_driver_frontier.json'
$evidencePath = Join-Path $repoRoot 'Projects\Codelation\driver_evidence\pi4_driver_gen17_verified.json'
foreach ($path in @($basePath, $extensionPath, $frontierPath, $evidencePath)) {
    if (-not (Test-Path -LiteralPath $path -PathType Leaf)) {
        throw "Missing Aurum Pi4 generation-18 prerequisite: $path"
    }
}

$verified = Get-Content -LiteralPath $evidencePath -Raw | ConvertFrom-Json
if ($verified.schema -ne 'aurum-pi4-driver-generation-evidence-v1' -or [int]$verified.generation -ne 17) {
    throw 'Generation-18 requires committed generation-17 physical evidence.'
}
if ($verified.target -ne 'gpio-leds' -or $verified.device_class -ne 'noncritical-indicator' -or $verified.candidate_driver -ne 'aurum-gpio-leds') {
    throw 'Generation-17 evidence does not match the approved noncritical gpio-leds lane.'
}
$requiredTrue = @(
    'compile',
    'candidate_bind',
...
foreach ($name in $requiredTrue) {
    if (-not [bool]$verified.verified.$name) {
        throw "Generation-17 evidence is missing required verified gate: $name"
    }
}
if ([int]$verified.verified.first_cycle_kernel_fault_signatures -ne 0 -or [int]$verified.verified.second_cycle_kernel_fault_signatures -ne 0) {
    throw 'Generation-17 evidence contains a kernel-fault signature.'
}
if ([int]$verified.verified.cycles_without_reboot -lt 2) {
    throw 'Generation-17 evidence does not prove two cycles without reboot.'
}
foreach ($name in @('raw_mmio','firmware_changes','network_changes','storage_changes','persistent_boot_time_replacement')) {
    if ([bool]$verified.verified.$name) {
        throw "Generation-17 evidence crossed a prohibited boundary: $name"
    }
}
...
$limitNew = 'elseif ($generation -eq $validated -and $next -eq ($validated + 1) -and $next -le 18) {'
if (-not $patched.Contains($limitOld)) {
    throw 'Could not extend the Aurum Pi4 autonomous next-generation limit to generation 18.'
}
$patched = $patched.Replace($limitOld, $limitNew)

$guardOld = 'if ($trialGeneration -lt 1 -or $trialGeneration -gt 10) {'
$guardNew = 'if ($trialGeneration -lt 1 -or $trialGeneration -gt 18) {'
if (-not $patched.Contains($guardOld)) {
    throw 'Could not extend the Aurum Pi4 supported-generation guard to generation 18.'
}
...
if ([int]$frontier.validated_generation -gt 17) {
    throw "Committed frontier unexpectedly exceeds generation 17: $($frontier.validated_generation)"
}
$frontier.active_generation = 17
$frontier.validated_generation = 17
$frontier.next_generation = 18
...
try {
    ...
    Write-Host 'AURUM_PI4_DRIVER_GEN18_PREREQUISITE generation17=verified two_cycles=true kernel_fault_signatures=0 forbidden_boundaries=false'
