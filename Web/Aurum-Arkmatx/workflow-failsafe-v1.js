...
let state={phase:'checking',observedAt:0,ageMs:null,active:[],stalled:[],unresolved:[],failures:[],lanes:[],readbackFiles:{active:null,history:null},liveConfirmed:false,externalGuiProofBoundary:false,humanActionInference:false,detail:'Checking workflow failsafe evidence…'};
...
function render(){
 ...
 const advancing=state.ageMs!==null&&state.ageMs<=SNAPSHOT_STALE?'durable active/history readback is current; terminal workflow reconciliation is active; pointer-path availability is kept separate from observed pointer motion':'no fresh frontier evidence claimed';
...
async function refresh(){
 ...
 try{
  ...
  state={phase,observedAt:observed,ageMs:age,liveConfirmed,active,stalled,unresolved,failures,lanes,readbackFiles:{active:activeFile.meta?.name||null,history:historyFile.meta?.name||null},externalGuiProofBoundary,humanActionInference:false,detail:'Failsafe workflow evidence loaded.'};
 }catch(e){state={phase:'unknown',observedAt:0,ageMs:null,liveConfirmed:false,active:[],stalled:[],unresolved:[],failures:[],lanes:[],readbackFiles:{active:null,history:null},externalGuiProofBoundary:false,humanActionInference:false,detail:e?.message||'request failed'}}
 render();publish();
}
