 ...
 * AURUM_WORKFLOW_FAILSAFE_V1_9_CANONICAL
 ...
 * Native-chain candidate/planning verification, expired adaptive-shell GUI or GUI-preference live-trial evidence, and physical-presence ownership stay separate.
 */
(()=>{
'use strict';
...
function nativeChainView(n){
 ...
 const planningVerified=blockedOutput==='blocked-planning-verified';
 const verified=n?.last_result?.status==='verified'||n?.promotion?.current_status==='verified'||blockedOutput==='blocked-candidate-verified'||planningVerified;
 const evidenceExpired=n.external_evidence_receipt_expired===true||String(n.rule_declared_evidence_state||'')==='expired'||['gui-live-trial-evidence-expired','gui-preference-trial-evidence-expired'].includes(externalReason);
 const physicalBlock=blockedOutput==='blocked-physical-node';
 const preferenceTrialGap=gap==='adaptive_shell_gui_preference_live_trial';
 const liveTrialGap=['adaptive_shell_live_trial_readiness','adaptive_shell_gui_live_trial','adaptive_shell_gui_preference_live_trial'].includes(gap);
 const liveTrialLabel=preferenceTrialGap?'adaptive-shell GUI preference live-trial':'adaptive-shell GUI live-trial';
 ...
 return{status,blockedOutput,gap,verified,planningVerified,evidenceExpired,physicalBlock,preferenceTrialGap,liveTrialLabel,liveTrialSystemBoundary,lastCompleted,nextGeneration,proposal:n.proposal_id||'unknown',reason:externalReason,nextTransition:n.next_safe_transition||'provide-rule-declared-evidence',nextTarget:n.next_safe_transition_target||gap};
}
function nativeNeeds(v){if(!v)return'';if(v.liveTrialSystemBoundary)return`Native chain generation ${v.lastCompleted} is complete and the current candidate/planning state remains verified, but ${v.liveTrialLabel} evidence expired. Aurum/System must reacquire fresh bounded ${v.liveTrialLabel} evidence through the rule-declared evidence path without reopening already-closed GUI/core gates. Physical presence is not the current blocker.`;if(v.status==='external-prerequisite-blocked')return`Native chain is externally blocked at ${String(v.gap).replace(/_/g,' ')}; ownership/root cause remains system-side unless separate current evidence proves a human-only boundary.`;return''}
function render(){
 ...
 if(state.phase==='checking'){pill.className='pill running';pill.textContent='Running';evidence.textContent='Checking latest durable readback and native-chain evidence…'}
  ...
  else if(nv?.liveTrialSystemBoundary)evidence.textContent=`Native chain generation ${nv.lastCompleted} complete · current candidate/planning state verified · ${nv.liveTrialLabel} evidence expired · bounded system-side evidence reacquisition required; physical presence is not the current blocker.`;
 ...
 if(nv?.verified)advancing+=`; native chain has ${nv.lastCompleted} completed generations and the current candidate/planning state remains verified`;
