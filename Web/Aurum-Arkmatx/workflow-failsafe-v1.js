 ...
 * Hopper Echo physical proof is tracked as a self-hosted hardware-proof workflow lane.
 * Queued self-hosted proof explicitly reports scheduler placement without guessing the root cause.
...
const CORE=[
 ...
 ['aurum-hopper-echo-proof.yml@main','Hopper Echo physical proof','self-hosted Linux'],
 ['ci.yml@main','Repository CI']
];
let state={phase:'checking',observedAt:0,ageMs:null,active:[],stalled:[],failures:[],lanes:[],readbackFiles:{active:null,history:null},detail:'Checking workflow failsafe evidence…'};
...
function publish(){window.__aurumWorkflowFailsafeState={...state};window.dispatchEvent(new CustomEvent('aurum-workflow-failsafe-state',{detail:{...state}}))}
...
function receiptRank(name,prefix){
 const m=String(name||'').match(new RegExp(`^${re(prefix)}-(\\d+)(?:-attempt-(\\d+))?\\.json$`,'i'));
 if(!m)return null;
 return{run:Number(m[1]),attempt:Number(m[2]||0)};
}
async function latest(prefix){
 const list=await json(RESULTS);
 const files=Array.isArray(list)?list.map(meta=>({meta,rank:meta?.type==='file'?receiptRank(meta.name,prefix):null})).filter(x=>x.rank).sort((a,b)=>(b.rank.run-a.rank.run)||(b.rank.attempt-a.rank.attempt)):[];
 if(!files.length)return null;
 return{meta:files[0].meta,rank:files[0].rank,payload:await rawJson(files[0].meta.download_url)};
}
...
function render(){
 ...
 else if(state.phase==='verified'){pill.className='pill success';pill.textContent='Verified';evidence.textContent=`Workflow failsafe readback is healthy · ${state.lanes.length} core lanes checked · no current stall.`}
 ...
 else if(state.phase==='attention'){pill.className='pill failed';pill.textContent='Attention';evidence.textContent=state.stalled.length?(state.stalled[0].kind==='queued'?`${queueSummary(state.stalled[0])} Aurum/system attention required.`:`${state.stalled[0].label} running beyond the one-hour execution grace · Aurum/system attention required.`):`${failureSummary(state.failures[0])} · Aurum/system attention required.`}
 ...
 detail.innerHTML=`<b>Snapshot observed:</b> ${esc(state.observedAt?new Date(state.observedAt).toLocaleString():'not recorded')}<br><b>Snapshot age:</b> ${esc(state.ageMs===null?'unknown':ageText(state.ageMs))}<br><b>Active readback:</b> ${esc(state.readbackFiles?.active||'not found')}<br><b>Run history:</b> ${esc(state.readbackFiles?.history||'not found')}<br><b>Active core lanes:</b> ${esc(state.active.length)}<br><b>Stalled core lanes:</b> ${esc(state.stalled.length)}<br><b>Recent failed core lanes:</b> ${esc(state.failures.length)}<br><b>Live run confirmation:</b> ${esc(Boolean(state.liveConfirmed))}<br><b>Failure detail:</b> ${esc(failureDetail)}<br><b>Scheduler placement:</b> ${esc(placementDetail)}<div class="wf-grid">${rows||'<span>No core lane evidence loaded.</span>'}</div><br><b>Interpretation:</b> this surface accepts both legacy failsafe receipts and rerun-safe <code>RUNID-attempt-N</code> receipts, chooses the newest run and attempt, live-checks non-completed run IDs before calling them stalled, enriches recent completed failures with failed job/step names, includes Hopper Echo physical proof, and lets live-confirmed terminal state override an older durable snapshot. A queued self-hosted workflow proves only that matching runner placement has not happened; it does not prove whether the runner is offline, busy, mislabeled, or otherwise unavailable.<br><b>Ownership:</b> runner availability, workflow dispatch, CI, controller, recovery, seed, route, hardware proof, and evidence freshness are Aurum/system work. Workflow state alone never creates a human task.`;
}
async function refresh(){
 ...
 try{
  ...
  const lanes=[];
  for(const[key,label,runnerClass]of CORE){
   ...
   let note='';if(run){const runAge=Math.max(0,Date.now()-ms(run.created_at));if(['queued','waiting','requested','pending'].includes(run.status))note=`queued ${ageText(runAge)}${runnerClass?` · requires ${runnerClass}; scheduler placement pending`:''}`;else if(run.status==='in_progress')note=`running ${ageText(runAge)}`;else if(run.failure_detail)note=`failed: ${run.failure_detail}`;else note=`updated ${ageText(Math.max(0,Date.now()-ms(run.updated_at||run.created_at)))} ago`;}
   lanes.push({key,label,runnerClass:runnerClass||null,run,note});
  }
  const liveConfirmed=lanes.some(x=>Boolean(x.run?.live_confirmed)),active=[],stalled=[],failures=[];
  for(const lane of lanes){const r=lane.run;if(!r)continue;const a=Math.max(0,Date.now()-ms(r.created_at));if(['queued','waiting','requested','pending'].includes(r.status)){if(a>QUEUE_GRACE)stalled.push({...lane,kind:'queued'});else active.push(lane)}else if(r.status==='in_progress'){if(a>RUN_GRACE)stalled.push({...lane,kind:'running'});else active.push(lane)}else if(['failure','timed_out','startup_failure','action_required'].includes(r.conclusion)&&a<=6*60*60*1000)failures.push(lane)}
  let phase='verified';if(liveConfirmed&&(stalled.length||failures.length))phase='attention';else if(age===null||age>SNAPSHOT_STALE)phase='unknown';else if(stalled.length)phase='attention';else if(active.length)phase='running';else if(failures.length)phase='attention';
  state={phase,observedAt:observed,ageMs:age,liveConfirmed,active,stalled,failures,lanes,readbackFiles:{active:activeFile.meta?.name||null,history:historyFile.meta?.name||null},detail:'Failsafe workflow evidence loaded.'};
 }catch(e){state={phase:'unknown',observedAt:0,ageMs:null,active:[],stalled:[],failures:[],lanes:[],readbackFiles:{active:null,history:null},detail:e?.message||'request failed'}}
 render();publish();
}
