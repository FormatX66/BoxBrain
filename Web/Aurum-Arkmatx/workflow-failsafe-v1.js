 ...
 * Native-chain candidate verification, expired adaptive-shell live-trial evidence, and physical-presence ownership stay separate.
 */
(()=>{
'use strict';
...
function nativeChainView(n){
 ...
 const evidenceExpired=n.external_evidence_receipt_expired===true||String(n.rule_declared_evidence_state||'')==='expired';
 const physicalBlock=blockedOutput==='blocked-physical-node';
 const liveTrialSystemBoundary=status==='external-prerequisite-blocked'&&gap==='adaptive_shell_live_trial_readiness'&&verified&&evidenceExpired&&!physicalBlock;
 return{status,blockedOutput,gap,verified,evidenceExpired,physicalBlock,liveTrialSystemBoundary,lastCompleted:n.last_completed_generation??'unknown',nextGeneration:n.next_generation_id??'unknown',proposal:n.proposal_id||'unknown',reason:n.external_evidence_reason||'unknown',nextTransition:n.next_safe_transition||'unknown',nextTarget:n.next_safe_transition_target||'unknown'};
}
function nativeNeeds(v){if(!v)return'';if(v.liveTrialSystemBoundary)return`Native chain generation ${v.lastCompleted} is complete and the current candidate remains verified, but adaptive-shell live-trial evidence expired. Aurum/System must ${String(v.nextTransition).replace(/-/g,' ')} with a fresh bounded evidence window; do not reopen already-closed core gates. Physical presence is not the current blocker.`;if(v.status==='external-prerequisite-blocked')return`Native chain is externally blocked at ${String(v.gap).replace(/_/g,' ')}; ownership/root cause remains system-side unless separate current evidence proves a human-only boundary.`;return''}
function render(){
 ...
 if(state.phase==='checking'){pill.className='pill running';pill.textContent='Running';evidence.textContent='Checking latest durable readback and native-chain evidence…'}
  ...
  else if(nv?.liveTrialSystemBoundary)evidence.textContent=`Native chain generation ${nv.lastCompleted} complete · current candidate verified · adaptive-shell live-trial evidence expired · bounded system-side reseed required; physical presence is not the current blocker.`;
 ...
 const nativeDetail=nv?`generation ${nv.lastCompleted} complete · next ${nv.nextGeneration} · candidate verified=${nv.verified} · blocked output=${nv.blockedOutput} · next gap=${nv.gap} · evidence expired=${nv.evidenceExpired} · physical-presence blocker=${nv.physicalBlock}`:`unavailable${state.nativeChainError?` · ${state.nativeChainError}`:''}`;
