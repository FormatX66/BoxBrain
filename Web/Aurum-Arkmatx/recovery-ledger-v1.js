...
let evidence={manifest:null,ledgerText:'',bridgeText:'',carrierText:'',flashText:'',legacyText:'',contractText:'',run:null,error:''};
...
function stateSnapshot(){
 const implemented=ledgerImplementationPresent(),carried=bridgeCarriesLedger(),offline=offlineCarrier(),writeGate=preExecutionWriteGatePresent(),legacyDisabled=legacyRawWriteBypassDisabled();
 return{
...
function patchDetail(){
 ...
 box.innerHTML=`<b style="color:#cfd3df">Recovery safety ledger + offline fallback</b><br><b style="color:#8ce7b2">Frontiers Advancing:</b> ${esc(checkpoint)} ${esc(fallback)} ${esc(gate)} ${esc(legacy)}<br><b style="color:#bbb6ff">Needs Work → Aurum/System:</b> ${esc(needs)}<br><b style="color:#f0c76a">Your Actions:</b> None from this evidence layer. The offline carrier and write-path guards never grant destructive write, boot, promotion, or recovery authority; Action Ownership remains the authority boundary.<br><b style="color:#74d8d0">Germ CI:</b> ${esc(germCiText())}`;
}
async function refresh(){
 try{
  const [m,l,b,c,f,x,k,r]=await Promise.all([
  ...
  if(!m.ok||!l.ok||!b.ok||!c.ok||!f.ok||!x.ok||!k.ok||!r.ok)throw new Error(`GitHub evidence HTTP ${m.status}/${l.status}/${b.status}/${c.status}/${f.status}/${x.status}/${k.status}/${r.status}`);
  const manifest=await m.json(),ledgerText=await l.text(),bridgeText=await b.text(),carrierText=await c.text(),flashText=await f.text(),legacyText=await x.text(),contractText=await k.text(),runs=(await r.json()).workflow_runs||[];
  evidence={manifest,ledgerText,bridgeText,carrierText,flashText,legacyText,contractText,run:newest(runs,GERM_WORKFLOW),error:''};
 }catch(e){evidence={manifest:null,ledgerText:'',bridgeText:'',carrierText:'',flashText:'',legacyText:'',contractText:'',run:null,error:e?.message||'request failed'}}
 augment();
}
