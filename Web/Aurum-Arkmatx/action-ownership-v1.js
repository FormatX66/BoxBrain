/* AURUM_ACTION_OWNERSHIP_V1_CANONICAL
 ...
 */
(()=>{'use strict';if(window.__aurumActionOwnershipV1)return;window.__aurumActionOwnershipV1=true;
...
const state={voice:null,native:null,voiceSource:'unknown',updated:0,lastRender:''};
...
function systemItems(){const out=[];const v=state.voice,n=state.native;
...
function render(){const gate=document.querySelector('#humanGate');if(!gate)return;const action=state.voice?.overall?.human_action||'Human-action evidence is unavailable.';const human=isHumanAction(action);const work=systemItems();
  const fingerprint=JSON.stringify([human,action,work,state.voiceSource]);if(fingerprint===state.lastRender)return;state.lastRender=fingerprint;
...
async function refresh(){await Promise.all([voice(),native()]);state.updated=Date.now();state.lastRender='';render()}
