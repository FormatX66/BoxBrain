 ...
 * AURUM_ACTION_OWNERSHIP_V1_9_CANONICAL compatibility marker.
 ...
 * Hopper live-status evidence is freshness-gated so a stalled observability runner cannot make an old
 * reachable/unreachable result look current or manufacture a human task.
 * Unknown or unavailable evidence never creates a human task.
 * Unknown evidence never becomes your task.
 */
(()=>{
'use strict';
if(window.__aurumActionOwnershipV110)return;
window.__aurumActionOwnershipV110=true;
...
function hopperStatusFresh(){const age=state.hopperStatusAgeMs;return Boolean(state.hopperStatus&&Number.isFinite(Number(age))&&age>=-5*60*1000&&age<=HOPPER_STATUS_MAX_AGE)}
...
function systemItems(){
 const out=[],v=state.voice,n=state.native,r=state.route,s=state.seed,q=state.reach,rg=state.recovery,pre=preExecutionState(),hs=state.hopperStatus;
 ...
 if(!hs){out.push({title:'Hopper live-status observability',detail:'The dedicated Hopper live-status snapshot is unavailable. Restore the read-only evidence lane before treating any prior Hopper health result as current. Missing evidence is Aurum/system work and cannot create a human task.'})}
 else if(!hopperStatusFresh()){out.push({title:'Hopper live-status observability stale',detail:`The last durable Hopper snapshot is ${ageText(state.hopperStatusAgeMs)} old (observed ${hs.observed_at||'at an unknown time'}). The status workflow is expected to preserve at least a 30-minute freshness heartbeat when its dedicated self-hosted lane is running. Diagnose/restore the hopper-status runner lane and produce a fresh read-only snapshot before treating the old ${hs?.summary?.health||'health'} result as current. Do not infer Hopper power state or assign a physical task from stale observability.`})}
 else if(hs?.reachable===false){out.push({title:'Hopper live endpoint read-only probe',detail:`A fresh Hopper status snapshot reports the live endpoint unreachable from ${hs.runner_host||'the status runner'}. Diagnose endpoint selection, networking, and runtime service health in the system. This evidence does not by itself prove Hopper is powered off or require human intervention.`})}
 if(recoveryHuman(rg)){
...
function render(){
 ...
 const fingerprint=JSON.stringify([actions,action,work,state.voiceFresh,state.voiceSource,state.route?.phase,state.route?.result?.observed_at,state.seed?.phase,state.seed?.receipt?.observed_at,state.reach?.phase,state.reach?.observedMs,recovery?.schema,recovery?.humanActionRequired,recovery?.humanActionEvidence,recovery?.tinySeedPhysicalHandoffPhase,recovery?.tinySeedReleaseState,recovery?.tinySeedReleaseSourceCommit,recovery?.tinySeedX86Sha256,pre,state.preExecution?.observed_at,state.preExecution?.remote_repair,state.hopperStatus?.observed_at,state.hopperStatus?.reachable,state.hopperStatus?.summary?.health,state.hopperStatusAgeMs]);
 if(fingerprint===state.lastRender)return;state.lastRender=fingerprint;
 gate.className=`gate ${human?'':'ok'}`;gate.dataset.actionContract='verified';gate.dataset.humanActionCount=String(actions.length);gate.dataset.systemWorkCount=String(work.length);gate.dataset.voiceEvidenceFresh=String(state.voiceFresh);gate.dataset.recoveryHumanBoundary=String(recoveryConfirmed);gate.dataset.recoveryHumanBoundaryReleased=String(recoveryReleased);gate.dataset.preExecutionGate=pre;gate.dataset.hopperStatusEvidenceFresh=String(hopperStatusFresh());
 gate.innerHTML=`<b>${human?'Human-only action confirmed.':state.voiceFresh?'No action needed from you right now.':'No confirmed action from you; available action evidence does not establish a human boundary.'}</b><p>${esc(action)}</p><div class="ao-grid"><div class="ao-box ${human?'human':'clear'}"><span class="ao-label">YOUR ACTIONS</span><strong>${human?`${actions.length} confirmed`:'0 confirmed'}</strong><small>${human?esc(actions.join(' ')):state.voiceFresh?'Nothing is currently assigned to you.':'Stale/unknown evidence is blocked from assigning you work unless an independent verified evidence source establishes a human-only boundary.'}</small></div><div class="ao-box system"><span class="ao-label">NEEDS WORK → AURUM/SYSTEM</span><strong>${work.length} system ${work.length===1?'item':'items'}</strong>${work.length?`<ul>${work.map(x=>`<li><b>${esc(x.title)}</b><br>${esc(x.detail)}</li>`).join('')}</ul>`:'<small>No structured system work found in these evidence sources.</small>'}</div></div><div class="ao-source">Action ownership uses voice evidence, the dedicated Hopper live-status snapshot, Recovery Guardian release/handoff evidence, the Future Branch pre-execution Hopper recovery receipt, native-chain state, seed receipts, dynamic BBPI4 reachability diagnostics, and end-to-end Hopper route proof. Voice/action evidence must be fresh (≤6h) before it can create a voice-derived assignment. Hopper live-status evidence must be fresh (≤1h) before its reachable/unreachable result is treated as current; stale observability stays system-owned. Recovery Guardian Tiny Seed actions are additionally blocked until a fresh terminal pre-execution recovery pass proves the safe already-authorized remote branch has been exhausted without completing recovery. Unknown evidence never becomes your task.</div>`;
...
async function hopperStatus(){try{const r=await fetch(`${HOPPER_STATUS}?t=${Date.now()}`,{cache:'no-store'});if(!r.ok)throw new Error(String(r.status));const p=await r.json();if(p?.schema!=='aurum.hopper.live-status.v2')throw new Error('schema');state.hopperStatus=p;const t=time(p?.observed_at);state.hopperStatusAgeMs=t?Date.now()-t:null}catch(_){state.hopperStatus=null;state.hopperStatusAgeMs=null}}
async function refresh(){await Promise.all([voice(),native(),preExecution(),hopperStatus()]);state.route=window.__aurumControlRouteState||state.route;state.seed=window.__aurumSeedDeliveryState||state.seed;state.reach=window.__aurumPi4ReachabilityState||state.reach;state.recovery=window.__aurumRecoveryGuardianState||state.recovery;state.updated=Date.now();state.lastRender='';render()}
