/* AURUM_ACTION_OWNERSHIP_V1_7_CANONICAL
...
(()=>{
'use strict';
if(window.__aurumActionOwnershipV17)return;
window.__aurumActionOwnershipV17=true;
...
function recoveryAction(r){if(!recoveryHuman(r))return'';const src=String(r?.tinySeedReleaseSourceCommit||'the verified handoff source');const sha=String(r?.tinySeedX86Sha256||'the verified x86 checksum');return`Tiny Seed physical handoff: connect a disposable/test USB that is not a boot/system disk and is not protected recovery media. Identify its exact serial. Run ${r?.dryRunFlashWorkflow||'Aurum Tiny Seed Windows Flash'} with write=false using source commit ${src}; only after preflight proves that serial and x86 SHA-256 ${sha} should you explicitly authorize the destructive write with write=true and the workflow-required confirmation text. Do not boot it until raw readback verification passes.`}
