/* AURUM_ACTION_OWNERSHIP_V1_1_CANONICAL
 * Canonical website owner: FormatX66/ClusterSites.
 * Separates confirmed human-only work from Aurum/system work using structured evidence.
 * Includes the read-only BoxBrain -> Hopper route frontier when that proof is pending or failed.
 * Unknown or unavailable evidence never creates a human task.
 */
(()=>{'use strict';if(window.__aurumActionOwnershipV11)return;window.__aurumActionOwnershipV11=true;
...
function systemItems(){const out=[];const v=state.voice,n=state.native,r=state.route;
  ...
  if(r?.phase==='pending'){
    out.push({title:'BoxBrain → Hopper control-route proof',detail:'A read-only GitHub → main PC → BBPI4 → Hopper proof was requested and is waiting for a recorded result. This is Aurum/system work, not a human action.'});
