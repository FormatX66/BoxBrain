...
const EVIDENCE_COMMIT='691823f5660334ed0bd7f2843c58a1a21db24821';
...
const generations=[
 ...
 {id:'Gen3',title:'Scoped inheritance + lineage',status:'CI-proven foundation',ready:['scope-bound trait inheritance','trusted-node + safety vetoes','immutable hash-linked lineage ledger','tamper/parent-fork rejection','durable quarantine evidence','cross-node evidence receipt + replay','multi-node exchange software preflight'],held:['multi-node live exchange','independent-node recovery','earned Gen2 parent']}
];
...
function enhance(){
 ...
 panel.innerHTML=`<h4>Future Branch · generation seed ladder</h4><p><b>Evidence rule:</b> later generations may be prepared in parallel, but preparation never makes a generation earned. Parent, recovery, provenance, and generation-specific external gates remain fail-closed.</p>${generations.map(generationHtml).join('')}<p><b>Frontiers Advancing:</b> BoxBrain main CI has proven an executable Gen1→Gen3 preparation ladder through <code>${esc(EVIDENCE_COMMIT.slice(0,12))}</code>: Gen1 readiness evaluation, Gen2 machine-native/Slush state, and Gen3 scoped inheritance, immutable lineage replay, deterministic cross-node evidence receipt/replay, and a zero-authority multi-node exchange software preflight.</p><p><b>Needs Work → Aurum/System:</b> earn Gen1 only after its physical/recovery/second-architecture gates; continue remaining Gen2 software/recovery work and take Gen3 from software preflight to authenticated live multi-node exchange plus independent-node recovery without inferring physical proof. No descendant may bypass an unearned parent, recovery proof, or provenance.</p><p><b>Your Actions:</b> none from generation preparation. CI, predicted futures, lineage data, exchange preflight, or a software-ready descendant cannot grant physical/destructive authority or create a human task.</p><p><a href="${EVIDENCE_URL}" target="_blank" rel="noopener">Open verified BoxBrain evidence head ↗</a></p>`;
}
let scheduled=false;
