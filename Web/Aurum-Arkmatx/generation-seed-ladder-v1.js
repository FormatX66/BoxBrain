...
const generations=[
 {id:'Gen1',title:'Everyone-OS readiness gate',status:'LKG restored · boot-verified base · broken human-surface candidate quarantined',ready:['graphical shell contract','seven everyday traits contract','intent/accessibility contract','recovery contract','unattended candidate validation','historical TR8:WEB implementation/acceptance contract at pinned evidence commit','historical TR8:PROMPT implementation/fallback contract at pinned evidence commit','forward-only active trunk restored byte-for-byte to LKG tree','restore-tree PC image build','generic low-memory UEFI install/reboot/self-build smoke','Hopper HP topology + offline-recovery twin smoke','boot-verified ISO publication'],held:['TR8:WEB payload on active trunk','TR8:PROMPT payload on active trunk','repaired WEB/PROMPT candidate seed/VM acceptance','visible browser render + keyboard/pointer acceptance','prompt runtime health/fallback acceptance','Hopper physical boot','Guardian forced rollback','second-architecture usability']},
